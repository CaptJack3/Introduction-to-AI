...
class Game:
    def __init__(self, board, players_positions):
        ...
        :param board: np.array. The initial board.
              board size is 24 (0-23). 0- free space. 1= player1, 2- player2
        :param players_positions: the initial players positions
    ...
    def check_move(self, move) -> bool:
        ...
        if not self.pos_feasible_on_board(move[0]):  # Check that the position is to an empty cell on board
            return False
        if self.players_positions[self.turn][
            move[1]] == -1:  # If soldier not on board yet -> you can put him on the free cell
            return True
        if len(np.where(self.players_positions[self.turn] == -1)[
                   0]) > 0:  # if soldier is on board -> check that all soldiers are already on board.
            return False
