"""
 ...
 THIS IS OLD THINS THAT WAS IN THE FILE
...
from SearchAlgos import Minimax,AlphaBeta, State
import time
import numpy as np
...
class Player(AbstractPlayer):
    def __init__(self, game_time):
        ...
        self.avg_N=35
        self.avg_N = 35
        self.avg_time=self.game_time/self.avg_N
    ...
    def make_move(self, time_limit) -> tuple:
        ...
        """
        # time limit for single move calculation:
        time_limit=self.turn_factor(self.turn,self.avg_N,self.avg_time)*self.avg_time
        ...
        while depth ==1 or time_end-time.time()>(iter_finish-iter_start)*self.BF+self.SF1:
            ...
            direction = search_tree.search(state, depth, self.turn, True,alpha,beta)[1]
            depth+=1
            iter_finish=time.time()
        # print for debug
        #     print("Minimax Move:", direction)
        self.set_my_move(direction) # update board with my move
        return direction
    ...
    def turn_factor(self,turn,N,avg):
        #  time_limit=self.turn_func(self.turn,self.avg_N,self.avg_time)*self.game_time/self.avg_N
        # N - average number of moves
        # t- average time on move = global_time/N
        # turn = turn
        exp_fact=(1/N)*np.log(1/avg)
        turn_fact=np.exp(exp_fact*turn)
        return turn_fact