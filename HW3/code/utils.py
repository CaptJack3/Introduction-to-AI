...
def accuracy(y: np.array, y_pred: np.array):
    ...
    n_correct_predictions = np.count_nonzero(y == y_pred)
    accuracy_val = n_correct_predictions / len(y)
...
def l2_dist(x1: np.array, x2: np.array):
    ...
    # n1_len = len(x1)
    # n2_len = len(x2)
    # dimensions = np.shape(x1)[-1]

    # dists = np.zeros([n1_len, n2_len])

    # for ind_1 in range(n1_len):
    #     x1_element = x1[ind_1, :]
    #     for ind_2 in range(n2_len):
    #         x2_element = x2[ind_2, :]
    #         dists[ind_1, ind_2] = np.linalg.norm(x1_element - x2_element,
    #                                              ord=2,
    #                                              axis=0)
    # return dists

    ####### Current implementation:more compact
    sx = np.sum(x1**2, axis=1, keepdims=True)
    sy = np.sum(x2**2, axis=1, keepdims=True)
    return np.sqrt(-2 * x1.dot(x2.T) + sx + sy.T)
...
def create_train_validation_split(dataset: np.array,
                                  kf: KFold,
...
if __name__ == "__main__":
    ...
    small_td = train_dataset.to_numpy()
    print(type(attributes_names), "\n", small_td[:5, :10])
